...
# = Generating Guids
#
# Call #generate to generate a new Guid. The method returns a string in one of
...
# adds the <tt>:urn:Guid</tt> prefix.
#
# For example:
#
# Guid = Guid.new
...
# = Guids in Brief
#
# Guid (universally unique identifier) are guaranteed to be unique across time
# and space.
#
# A Guid is 128 bit long, and consists of a 60-bit time value, a 16-bit
...
# each time the Guid generator is started. The combination guarantees that
# identifiers created on the same machine are unique with a high degree of
# probability.
#
# Note that due to the structure of the Guid and the use of sequence number,
# there is no guarantee that Guid values themselves are monotonically
# incrementing. The Guid value cannot itself be used to sort based on order
# of creation.
#
# To guarantee that Guids are unique across all machines in the network,
...
class Guid
  ...
  ##
  # Clock multiplier. Converts Time (resolution: seconds) to Guid clock
  # (resolution: 10ns)
  ...
  ##
  # Version number stamped into the Guid to identify it as time-based.
  VERSION_CLOCK = 0x0100
 
  ##
  # Formats supported by the Guid generator.
  #
  # <tt>:default</tt>:: Produces 36 characters, including hyphens separating
  # the Guid value parts
  # <tt>:compact</tt>:: Produces a 32 digits (hexadecimal) value with no
  # hyphens
  # <tt>:urn</tt>:: Adds the prefix <tt>urn:Guid:</tt> to the default format
  FORMATS = {
    :compact => '%08x%04x%04x%04x%012x',
    :default => '%08x-%04x-%04x-%04x-%012x',
    :urn => 'urn:Guid:%08x-%04x-%04x-%04x-%012x',
  }
  ...
  ##
  # Generates a new Guid string using +format+. See FORMATS for a list of
  # supported formats.
 
  def self.generate(format = :default)
    @Guid ||= new
    @Guid.generate format
  end
 
  ##
  # Creates an empty state file in /var/tmp/ruby-Guid or the windows common
  # application data directory using mode 0644. Call with a different mode
  # before creating a Guid generator if you want to open access beyond your
  # user by default.
  #
  # If the default state dir is not writable, Guid falls back to ~/.ruby-Guid.
  #
  ...
  def self.state_file(mode = 0644)
    ...
    if File.writable?(state_dir) then
      @state_file = File.join(state_dir, 'ruby-Guid')
    else
      @state_file = File.expand_path(File.join('~', '.ruby-Guid'))
    end
  ...
  ##
  # Create a new Guid generator. You really only need to do this once.
  def initialize
    @drift = 0
  ...
  def generate(format = :default)
    ...
    raise ArgumentError, "invalid Guid format #{format.inspect}" unless template
 
    # The clock must be monotonically increasing. The clock resolution is at
    # best 100 ns (Guid spec), but practically may be lower (on my setup,
